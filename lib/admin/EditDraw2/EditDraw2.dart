...
class EditDraw2 extends StatefulWidget {
  EditDraw2({Key? key,required this.drawRef}) : super(key: key);
  DocumentReference drawRef;

  @override
  State<EditDraw2> createState() => _EditDraw2State();
}
class _EditDraw2State extends State<EditDraw2> {
  ...
  Future getDraw()async{
    Map<String,dynamic> draw = {};
    await widget.drawRef.get().then((value){
      draw = value.data() as Map<String,dynamic>;
    });
    nameC.text = draw['name']??"";
    priceC.text = draw['value'].toString();
    url = draw['image'];
    selectedDateTime = draw['endTime'].toDate();
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text("Create Draw Mode2",style: FlutterFlowTheme.of(context).titleMedium,),
      ),
      body: Form(
        key: formK,
                ...
                onTap: ()async{
                  ...
                  if(formK.currentState!.validate()){
                    if(selectedDateTime!.compareTo(DateTime.now().add(Duration(minutes: 1)))>=1){
                      editDraw2(nameC.text, int.parse(priceC.text), selectedDateTime!,url!);
                    }
                    else{
  ...
  Future editDraw2(String name,int value, DateTime dateTime,String img)async{

    widget.drawRef.update({
      'name': name,
      'value': value,
      'image': img,
      'endTime': dateTime,
