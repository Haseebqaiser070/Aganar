...
class CreateDrawMode3 extends StatefulWidget {
  const CreateDrawMode3({Key? key}) : super(key: key);

  @override
  State<CreateDrawMode3> createState() => _CreateDrawMode3State();
}
class _CreateDrawMode3State extends State<CreateDrawMode3> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text("Elige tu numero de la suerte",style: FlutterFlowTheme.of(context).titleMedium,),
      ),
      body: Form(
        key: formK,
                ...
                onTap: ()async{
                  ...
                  if(formK.currentState!.validate()){
                    if(selectedDateTime!.compareTo(DateTime.now().add(Duration(minutes: 1)))>=1){
                      createDraw3(nameC.text, int.parse(priceC.text), selectedDateTime!,url!);
                    }
                    else{
  ...
  Future createDraw3(String name,int value, DateTime dateTime,String img)async{
    FirebaseFirestore firestore = FirebaseFirestore.instance;
    DocumentReference documentReference = firestore.collection("Sorteo3").doc();
